...
public class CircuitBreakerRetryTest extends Arquillian {
     ...
     * <p>
     * Test that when retries are configured with sufficient delay, a call to an open circuit can retry until the
     * circuit half-closes, allowing the call to succeed.
     */
    @Test
    public void testRetriesSucceedWhenCircuitCloses() {
        ...
        // The retry delay is 100ms, maxRetries is 20 so we expect a successful call after around 1 second when the
        // circuit closes
    ...
    public void testCircuitOpenWithFewRetriesAsync() {
        ...
        try {
            ...
            // serviceB should retry until the Retry maxRetries is reached. At that point a TestException
            // should be thrown. Assert if this does not happen.
    ...
    public void testCircuitOpenWithMultiTimeoutsAsync() {
        ...
        try {
            result.get(TCKConfig.getConfig().getTimeoutInMillis(10000), TimeUnit.MILLISECONDS); // Expected to finish
                                                                                                // after < 1 second
